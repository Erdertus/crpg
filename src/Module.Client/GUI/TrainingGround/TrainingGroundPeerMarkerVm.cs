...
public class TrainingGroundPeerMarkerVm : ViewModel
{
    ...
    private string _acceptDuelRequestText = string.Empty;
    private string _sendDuelRequestText = string.Empty;
    private string _waitingForDuelResponseText = string.Empty;
    ...
    public TrainingGroundPeerMarkerVm(MissionPeer peer)
    {
        TargetPeer = peer;
        _isEnabled = true;
        TargetIconType iconType = MultiplayerClassDivisions.GetMPHeroClassForPeer(TargetPeer).IconType;
        _compassElement = new MPTeammateCompassTargetVM(iconType, Color.White.ToUnsignedInteger(), Color.White.ToUnsignedInteger(), BannerCode.CreateFrom(new Banner()), isAlly: true);
        RefreshValues();
    }
    ...
    public override void RefreshValues()
    {
        ...
        _acceptDuelRequestText = new TextObject("{=tidE1V1k}Accept duel").ToString();
        _sendDuelRequestText = new TextObject("{=YLPJWgqF}Challenge").ToString();
        _waitingForDuelResponseText = new TextObject("{=MPgnsZoo}Waiting for response").ToString();
    }
